<div class="my-3 my-sm-3">
    <div class="date-ranger-picker">
        <input class="form-control text-center removeFocus" id="quoteDateRangePicker" type="text" placeholder="Select Date Range">
    </div>
</div>
    ...
    <div class="ms-0 ms-md-2">
        <div class="dropdown d-flex align-items-center me-2">
            ...
            <div class="dropdown-menu py-0" aria-labelledby="quoteFilterBtn">
                ...
                <div class="p-5">
                    @php
                        $statusArr = collect($filterHeads[0])
                            ->sortBy('key')
                            ->reverse()
                            ->toArray();
                    @endphp
                    <div class="mb-5">
                        <label for="quoteStatus" class="form-label">
                            {{ __('messages.common.status') . ':' }}
                        </label>
                        {{ Form::select('status', getTranslatedData($statusArr), \App\Models\Quote::STATUS_ALL, ['id' => 'quoteStatus', 'class' => 'form-select']) }}
                    </div>
                    <div class="mb-5">
                        <label for="clientFilter" class="form-label">
                            {{ __('messages.quote.client') . ':' }}
                        </label>
                        <select id="clientFilter" wire:model="selectedClient" class="form-select">
